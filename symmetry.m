...
% 3-6-12
% Ken Hwang
% Gilmore Lab, PSU, SLEIC, Dept. of Psych.
%
% Symmetry display script
% Usage: symmetry
% Strictly for display purposes (use param_setup for pattern
% paramterization).
...
%fix_change = [0; cellfun(@(x,y)(sum(x==y) < 3),pres_cell(1:end-1,4), pres_cell(2:end,4))]; % Calculating change in color (first is no change, 0)
...
while strcmp('on', get(t,'Running'))
    ...
    if keyIsDown
        ...
        elseif keyCode(cKey)
            ...
            if pres_cell{get(t,'TasksExecuted') + 1, 4} % Accounting for initial (non-timer) flip
                Hit = 1;
