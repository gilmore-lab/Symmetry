...
%   temp_1 = 4 
%   function params = symmetry_jps_5( temp_1 )
%%  close all; clear all; pack; clc; symmetry_jps_5( 3 );
...
try
    ...
    numberOfTasks = 1;
    numberOfTrials = 2;  % 2 per Task (i.e., 4 min )
        ...
        while( tempCounter < 1 ) %         
            if( useMCC_Flag )
                ...
                tempDIOvalue = 0;
                    tempValue = 0;
                    tic;
                    while( toc < 4 ) % Assuming 2 second TR pulse
                    end; 
                    toc
                tempFlag = 1;
            end % END - if( useMCC_Flag )
            if(  (tempDIOvalue == 0 ) && (tempFlag == 1) )
               tempCounter = tempCounter + 1;
               tempFlag = 0;
               beep;
           elseif(  (tempDIOvalue == 0)  )
                         tempFlag = 0;
           else  % ELSE - if( tempFlag == 10 )
                        tempFlag = 1;
           end  % END - if(  (tempDIOvalue == 0 ) && (tempFlag == 1) )
        end  % END - while( tempCounter < 3 )
       ...
       if(  rand < 0.25  )
        ...
        if( keyIsDown )
           ...
           if(  keyCode(aKey) || keyCode(bKey) || keyCode(cKey) || keyCode(dKey)  )
                ...
                tempKeyPressed = KbName( KbName(keyCode) );
        ...
        responseGripMatrix( tempCounter_2, : ) = [ fixationPointChangeFlag, tempKeyPressed ];
        tempCounter_2 = tempCounter_2 + 1;
                 
    %                    draw_fixation( params.display, fix );
    %                     Screen('FillOval', windowPointer, white, fix_coords) ;	% draw fixation dot (flip erases it
if(  1  )
        if (display.doublebuffer==1)
            vbl=Screen('Flip', windowPointer, vbl + (fps*on_secs-0.5)*ifi);
        end;

        Screen('FillRect', windowPointer, gray);
        %   draw_fixation( params.display, fix );
        % Colors the entire windowPointer gray after on cycle time
        if (display.doublebuffer==1)
            vbl=Screen('Flip', windowPointer, vbl + (fps*off_secs-0.5)*ifi);
        end;
end % END - if(  1  )
                    end
    ...
    if( useMCC_Flag )
        ...
        tempCounter_2 = 1;
        while( notSaved )
            tempFilename = ['Response/responseGripMatrix_', num2str( tempCounter_2 ), '.mat'];
            if(   exist( tempFilename )  )
                tempCounter_2 = tempCounter_2 + 1;
                notSaved = 1;
            else
                eval(  ['save ', tempFilename, ' responseGripMatrix']  );
                notSaved = 0;
                tempCounter_2 = 1;
            end;  % END - if(   exist( tempFilename )  )         
            %  save responseGripMatrix.mat responseGripMatrix
