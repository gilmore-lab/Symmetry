...
temp_1 = 4 
  ...
  % close all; clear all; clc; params = symmetry_jps_5( 3 )
...
numberOfTasks = 5; % NOTE! 1 => 2 repeats
        ...
        while( tempCounter < 1 ) %         
            if( useMCC_Flag )
                ...
                vbl = Screen( 'Flip', windowPointer  );
                    ...
                    tempValue = 0;
                    delay4sec = tic;
                    while( toc( delay4sec ) < 4 ) % Assuming 2 second TR pulse
...
if(  1  )
                        ...
                        if( useMCC_Flag )
                            ...
                            tempCounter_3 = tempCounter_3 + 1;
                            while(  toc( startImageDisplay ) < 1.98  ) % image is displayed for 2 seconds % Assuming TR = 2sec => 1 pulse
                            end;  % END - while( toc( startImageDisplay ) < 1.98 )
                        end % END - if( useMCC_Flag )                         
end % END - if(  0  )
...
if(  0  )
    ...
    toc( totalTaskTime )
        ...
        notSaved = 1;
        tempCounter_2 = taskIndex;
        while( notSaved )
            tempFilename = ['Response/Subject_', num2str( subject_ID ) , '/responseGripMatrix_TaskNumber_', num2str( tempCounter_2 ), '.mat'];
