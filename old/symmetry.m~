...
% Loading symmetry_cfg.mat
load ([file_dir filesep 'symmetry_cfg.mat'])
...
% Defining presentation defaults
...
for b = 1:expt.n_blocks
    ...
    seq_replications = default.seq_replications; % 1
    ...
    n_elements          = default.n_elements; % 6
    ...
    block(b).stim_secs  = repmat( [ default.class_duration ], n_patterns*seq_replications ); % 12, 1
    block(b).element_secs = block(b).stim_secs./block(b).n_elements; %12/6
...
try
    ...
    cKey = KbName('c');
    dKey = KbName('d');
    behavioralResponseStructure = struct( [] ); % Initializing behavioral response
...
for taskIndex = 1:numberOfTasks
    ...
    beginTriggerPulseTimer = tic; % Timing for start
    ...
    if( useMCC_Flag )
        ...
        Screen('FillRect', windowPointer, gray);
        DrawFormattedText(windowPointer, 'Waiting for trigger...', 'center', 'center', params.default.TextColor );
        vbl = Screen( 'Flip', windowPointer  );
        ...
        Screen(  'FillRect', windowPointer, gray  );
        DrawFormattedText(windowPointer, 'Waiting for Scanner Operator.', 'center', 'center', params.default.TextColor );
        vbl = Screen( 'Flip', windowPointer  );
        ...
        Screen('FillRect', windowPointer, gray); 
        DrawFormattedText(windowPointer, 'Pressed!', 'center', 'center', params.default.TextColor );
        vbl = Screen( 'Flip', windowPointer  );
        
%         tempDIOvalue = 0;
%         tempValue = 0;

        WaitSecs(4.75); % Assuming 2 second TR pulse      
...
%         end;  % END - while( toc( delay4sec ) < 4 )
    ...
    toc( beginTriggerPulseTimer ) % Reporting elasped time
    
    %% END - Delay start of display for 2 sync pulses

    totalRunTime = GetSecs; % Recording run time
...
%% BEGIN - Plot fixation point
       ...
       if(  rand < 0.3333 )
               ...
               %% BEGIN - Change fixation point color
                ...
                if(  ~strcmp(fixationPointShapeFlag,previousPointShape) || ~strcmp(fixationPointColor, previousPointColor)  );
...
%% END - Plot fixation point                        
 ...
 %% BEGIN - Keyboard check                                   
        ...
        behavioralResponseStructure( b ).StimulusType = tempStimulusType;                                
        behavioralResponseStructure( b ).TotalRunTime = GetSecs - totalRunTime;                                
        behavioralResponseStructure( b ).fixationPointChangeFlag = fixationPointChangeFlag;                                
        behavioralResponseStructure( b ).keyPressed = tempKeyPressed;                                
        behavioralResponseStructure( b ).keyPressTime = keyPressTime;                                
...
%% BEGIN - Keyboard check   
        ...
        b = b + 1;
                                ...
                                end  % END - if( keyIsDown && ~triggeredFlag)
...
if(  0  )
    ...
    toc( totalTaskTime )
        ...
        tempFolderName = ['Response' filesep 'Subject_', num2str( subject_ID )];
        ...
        notSaved = 1;
         tempCounter_10 = 1;
        while( notSaved )
            tempFilename = ['Response' filesep 'Subject_', num2str( subject_ID ) , filesep 'responseGripMatrix_TaskNumber_', num2str(  tempCounter_10 ), '.mat'];
